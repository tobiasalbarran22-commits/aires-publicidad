 ...
 */
// Fuerza el token explícito en vez de dejar que el SDK elija automático:
// en producción, si además de este token existe BLOB_STORE_ID (Vercel lo
// puede inyectar solo para stores linkeados), el SDK prefiere autenticar
// por OIDC en vez de este token — y ese camino se comportó distinto
// (escrituras que no se veían reflejadas en llamadas posteriores desde la
// misma función). Fijar el token evita esa ambigüedad.
const BLOB_TOKEN = process.env.BLOB_READ_WRITE_TOKEN;

async function readJSON(pathname, fallback) {
  try {
    const meta = await head(`data/${pathname}`, { token: BLOB_TOKEN });
    // Además de downloadUrl, se suma un parámetro con el propio ETag: así
    // cada versión real tiene una URL distinta, y ningún cache intermedio
    // puede confundir "la misma URL de siempre" con contenido viejo.
    const bustUrl = `${meta.downloadUrl}&v=${encodeURIComponent(meta.etag)}`;
